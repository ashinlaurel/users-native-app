...
const EditLectionary = ({ navigation, route }) => {
  const {key,obj} = route.params;
  const [thecount, setTheCount] = useState(obj.lessons.length);
  ...
  const [lessons, setLessons] = useState(obj.lessons);
  useEffect(() => {
    console.log("OBJ",obj.lessons.length)
    // setLessons(obj.lessons)
  //   var docRef = db.collection("lectionary").doc(key);
  ...
  const sendUser = async (values) => {
    ...
    db.collection("lectionary").doc(key)
      .update(values)
      .then(() => {
        console.log("updated");
        Alert.alert("Lectionary Updated");
